...
rule call_variants:
	...
	shell:
		...
		# --max-depth 500: use at most 500 reads per input BAM file, apparently these are sampled RANDOMLY!?
		...
		bcftools mpileup -Ou -f {input.ref} -R {input.regions}.gff_intersected.bed --bam-list <( ls mapped_reads/*.bam ) --max-depth 500 --min-MQ 20 --min-BQ 15 --no-BAQ -a INFO/AD -a FORMAT/AD | bcftools call -m --variants-only --skip-variants indels -Ov | bgzip -c > {output}
